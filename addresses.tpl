...
{if isset($multipleAddresses) && $multipleAddresses}
 <div class="addresses">
	...
	<div class="clearfix main-page-indent">
		<a href="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" title="{l s='Add an address'}" class="btn-circle btn-circle-raised btn-circle-primary pull-right"><i class="fa fa-plus"></i></a>
	</div>
	...
	{foreach from=$multipleAddresses item=address name=myLoop}
    	<div class="col-xs-12 col-sm-4 address">
			...
			<ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if}{if $smarty.foreach.myLoop.index % 2} alternate_item{else} item{/if} box">
                ...
                <a class="btn btn-raised btn-primary" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html':'UTF-8'}" title="{l s='Update'}"><span>{l s='Update'}<i class="icon-chevron-right right"></i></span></a>
                <a class="btn btn-raised btn-danger" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}" data-id="addresses_confirm" title="{l s='Delete'}"><span>{l s='Delete'}<i class="icon-remove right"></i></span></a></li>
            </ul>
        </div>
			</div>
		</div>
...
<ul class="footer_links clearfix">
	<li><a class="btn-circle btn-circle-raised btn-circle-primary" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">{*<span><i class="icon-chevron-left"></i> {l s='Back to your account'}</span>*}<i class="zmdi zmdi-chevron-left"></i></a>
	<a class="btn-circle btn-circle-raised btn-circle-success" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}">{*<span><i class="icon-chevron-left"></i> {l s='Home'}</span>*}<i class="zmdi zmdi-home"></i></a></li>
</ul>
